#    Copyright 2026 Ewan Phillips, Benjamin Lindner and Holger Kantz
#
...
"""
This script contains the core functions to generate
simulations and aid with plotting
"""
...
def labels(ax, osc=True, ylab='theta', ylim=None):
    ...
    if osc==True:
        ...
        if ylab=='theta':
            ax.set_ylabel(r'$p_{s}(\theta)$')
        elif ylab=='theta_x':
            ax.set_ylabel(r'$p_{s}(\theta), p_{s}(x)$')
...
def det_mean(om, D1, T, f_, g, dt=0.001, xinit=10, 
           n=0, m=1, osc=False, steps=1, meth='ito'):
    ...
    if meth=='ito': print("still to do")

    if meth=='strat':
        for i in range(n_):
            dx = dt * (f_(X, om, n) + 0.5*g_mon_der(X, D1_, m)*g(X, D1_, m)) 
            X += dx
...
@jit(nopython=True)
#def stat_den_sde_strat(om, D1, D2, T, f_, g, dt, osc=False, Nb=300, low=0, lim=np.pi, xinit=0.1, n=0, m=1, start=50000): # lam=0,
def stat_den_sde_gen(a1, a2, D1, D2, T, f_, g, dt, osc=False, Nb=300, low=0, lim=np.pi, xinit=0.1, n=0, m=1, start=50000): # lam=0,
...
@jit(nopython=True)
def p0_ice(D1, D2, T, f, g, dt, Nb=300, rb=2): 
...
def anal_fpe(Dm_list, file_, ylim=None, save=None):
    ...
    for i in range(len(Dm_list)):
        df=pd.read_csv("/Users/tphillips/multiplicative_noise/c++/DATA/"+file_+".dat",sep=" ",header=None)
    ...
    if save: 
        f.savefig('/Users/tphillips/multiplicative_noise/pics/anal_FPE_om1_Dm1_Daddvaried.png', bbox_inches='tight', format='png', dpi=120)
...
def num_an_fpe_2(om, D1, D2, T, f_, g, file_, D1a=None, D2a=None, dt=0.01, osc=True, ylim=None, save=None, Nb=300):

    sns.set_theme(style="white", rc=params)
    f, ax = plt.subplots(1, 1, figsize=(8, 4))

    x, Pl = stat_den_sde(om, D1, D2, T, f_, g, dt, osc, Nb=Nb)
    ax.plot(x, Pl, '-', markersize=1, label=r"Num. $D_1=$"+str(D1))    

    df=pd.read_csv("/Users/tphillips/multiplicative_noise/c++/DATA/"+file_+".dat",sep=" ",header=None)
    df = df.dropna(axis='columns',how='all')
    df.columns=['phi','D1','D2','P', 'Pn', 'I', 'PdB']
    if D1a: df = df[df['D1'] == D1a]
    if D2a: df = df[df['D2'] == D2a]

    y = np.array(df['P'])
    y /= (np.mean(y)*2*np.pi)  # normalize y
    x = np.arange(-np.pi, np.pi, 2*np.pi/len(y))
    ax.plot(x, y, '-', markersize=1, alpha=0.6, label=r'Anal. $D_1 = {}$'.format(D1a))
...
def burst_counter(om, D1, D2, T, f, g, dt, a2=0, n_=0, m_=1, X0=0, a=-np.pi/2,
                  b=np.pi/2, act='ref', steps=1, meth='ito'): 
...
def num_burst(D_dm, om, Dadd, f_, g, dt, T, a2=0, n_=0, m_=[1], init=0, ref=False, 
    ...
    if (len(D_dm) > len(Dadd)) and (len(D_dm) > len(m_)):
        ...
        #ax.plot(D_dm, b_dm_num, ll[num], markersize=ms, mec = 'k') # , label=lab1
        ax.set_xlabel(r'$D_{\text{m}}$')
    elif (len(m_) > len(Dadd)) and (len(m_) > len(D_dm)):
        ax.plot(m_, b_dm_num, ol[num], markersize=ms, mec = 'k', label=lab1)
        ax.set_xlabel(r'$m$')
        D_dm = m_
    else:
        ax.plot(Dadd, b_dm_num, ol[num], markersize=ms, mec = 'k', label=lab1)
        ax.set_xlabel(r'$D_2$')
        D_dm = Dadd

    if ref:
        ax.plot(D_dm, b_dm, 'ro', markersize=ms, mec = 'k', label='sin. num. abs.')

    ax.tick_params(bottom=True, left=True)  
    ax.tick_params(direction="in", length=4, width=1, grid_alpha=0.5) 
    ax.legend(framealpha=0.93, fontsize=17, loc='upper right')
...
def num_an_burst(D_dm, om, Dadd, f_, g, dt, T, n_=0, m_=[1], a2=0, init=0,  
    ...
    for i in D_dm:
        for j in Dadd:
            ...
            if a==0:   # a=0 is used to indicate reflecting boundaries
                if np.array(df2['m']):  # m represents Tr here
    ...
    if save: 
        f.savefig('/Users/tphillips/multiplicative_noise/pics/'+save+'.png', bbox_inches='tight', format='png', dpi=140)
...
def exp_dec_dist(Dm, Da, f_, g_, n=0, m=1, T=5000000, X=0, height=np.pi/4, 
    ...
    while len(y) < 1000 and T2<5*delT: 
        #y = burst_hist(1, Dm, Da, T+T2, f_, g_, n=n, X=X, m=m, dt=dt, height=height)
        y += burst_hist(1, Dm, Da, T+T2, f_, g_, n=n, X_in=X, m=m, dt=dt, height=height)
        y = [x for x in y if x > 2.8]
        # I should augment the list for every extra delT
...
def exp_dec_dist2(Dm, Da, f_, g_, n_=5000000, n=0, m=1, X=0, height=np.pi/4, dt=0.001, plot=True):

    y = []
    y = burst_hist2(1, Dm, Da, n_, f_, g_, n=n, m=m, X_in=X, dt=dt, height=height)
    y= [x for x in y if x > 2.8]
...
@jit(nopython=True)
def stat_den_strat_s(om, D1, D2, T, f_, g, dt, Nb=300, 
...
@jit(nopython=True)
def stat_den_strat_l(om, D1, D2, T, f_, g, dt, osc=False, Nb=300, 
                     low=0, lim=np.pi, xinit=0.1, n=0, m=1, start=50000, 
                     sig=0, lin='gen'): 
    '''For linear model'''
...
def max_fwhm(om0=1, sig=-1, D1_inc=0.4, D2=0, T=10000, dt=0.001, # lim=np.pi
...
def max_fwhm_om(om0=1, sig=-1, om_inc=0.4, D2=0, T=10000, dt=0.001, # lim=np.pi
             dt_l=0.001, r=21, D0=0, v_sig=False, show=True, nu=1, lim=np.pi):
    om_list = []
    p_max4, p_max_lin = [], []
    fwhm_l, fwhm_, max_an = [], [], []
...
def plot_max_fwhm(Dlist, p_max_osc, p_max_lin, fwhm_l, fwhm_, 
    ...
    if save: 
        f.savefig('/Users/tphillips/multiplicative_noise/pics/max_fwhm'+s_lin+'sin_'+save+'.png', bbox_inches='tight', format='png', dpi=160*s_)
...
def plot_stat_dis(x_osc, P_osc, x_lin, P_lin, ax=None, f=None,
    ...
    if f != None:
        ...
        f.savefig('/Users/tphillips/multiplicative_noise/pics/'+save+'.png', bbox_inches='tight', format='png', dpi=150*s_)
        f.savefig('/Users/tphillips/multiplicative_noise/pics/'+save+'_2.png', bbox_inches='tight', format='png', dpi=90*s_)
